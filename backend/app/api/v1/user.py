...
async def get_system_settings(current_user: dict = current_user_dependency):
    """
    Get system settings for the current user.

    Returns dynamic system settings that are relevant for user profile functionality,
    including clothing categories, style importance levels, supported currencies,
    and default packing methods.
    """
    try:
        ...
        logger.error("Get system settings error: %s", type(e).__name__)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_public_system_settings():
    ...
    Returns only public system settings that don't require authentication.
    This endpoint can be used for initial app configuration.
    """
    try:
        public_settings = await system_settings_service.get_public_settings()
        ...
        logger.error("Get public system settings error: %s", type(e).__name__)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve public system settings",
        ) from e
